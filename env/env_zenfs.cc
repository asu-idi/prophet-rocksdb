...
#ifndef ROCKSDB_NO_DYNAMIC_EXTENSION
class ZenfsDynamicLibrary : public DynamicLibrary {
 public:
  ZenfsDynamicLibrary(const std::string& name, void* handle)
      : name_(name), handle_(handle) {}
  ~ZenfsDynamicLibrary() override { dlclose(handle_); }
...
class ZenfsClock : public SystemClock {
 public:
  static const char* kClassName() { return "ZenfsClock"; }
...
class ZenFSEnv : public CompositeEnv {
 public:
  static const char* kClassName() { return "ZenFSEnv"; }
  ...
  ~ZenFSEnv() override {
    if (this == Env::Default()) {
      ...
      // ZenFSEnv instances use the same thread_status_updater_, so never
  ...
  Status LoadLibrary(const std::string& name, const std::string& path,
    ...
    if (name.empty()) {
      ...
      if (hndl != nullptr) {
        result->reset(new ZenfsDynamicLibrary(name, hndl));
        return Status::OK();
      }
    } else {
      ...
      if (path.empty()) {
        ...
        if (hndl != nullptr) {
          result->reset(new ZenfsDynamicLibrary(library_name, hndl));
          return Status::OK();
        }
      } else {
        ...
        while (getline(ss, local_path, kPathSeparator)) {
          if (!path.empty()) {
            ...
            if (hndl != nullptr) {
              result->reset(new ZenfsDynamicLibrary(full_name, hndl));
              return Status::OK();
            }
          }
        }
  ...
  ZenFSEnv(const std::string& bdevname);

 private:
  friend Env* Env::Default();

  // The below 4 members are only used by the default ZenFSEnv instance.
...
std::shared_ptr<FileSystem> getDefaultFS(const std::string& bdevname){
  std::shared_ptr<FileSystem> fs;
  FileSystem* fs_ptr;
  Status s = ROCKSDB_NAMESPACE::NewZenFS(&fs_ptr, bdevname);
  assert(s.ok());
  fs.reset(fs_ptr);
  return fs;
}

ZenFSEnv::ZenFSEnv(const std::string& bdevname)
    : CompositeEnv(getDefaultFS(bdevname), SystemClock::Default()),
...
void ZenFSEnv::Schedule(void (*function)(void* arg1), void* arg, Priority pri,
                        void* tag, void (*unschedFunction)(void* arg)) {
  assert(pri >= Priority::BOTTOM && pri <= Priority::HIGH);
  thread_pools_[pri].Schedule(function, arg, tag, unschedFunction);
}

int ZenFSEnv::UnSchedule(void* arg, Priority pri) {
  return thread_pools_[pri].UnSchedule(arg);
}

unsigned int ZenFSEnv::GetThreadPoolQueueLen(Priority pri) const {
  assert(pri >= Priority::BOTTOM && pri <= Priority::HIGH);
...
void ZenFSEnv::StartThread(void (*function)(void* arg), void* arg) {
  pthread_t t;
